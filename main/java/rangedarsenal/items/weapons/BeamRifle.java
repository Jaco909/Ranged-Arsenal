...
import rangedarsenal.events.BeamRifleEvent;

import java.awt.*;
import java.awt.geom.Point2D;
import java.util.LinkedList;

public class BeamRifle extends GunProjectileToolItem implements ItemInteractAction {
    public BeamRifle() {
        super(NORMAL_AMMO_TYPES, 1500);
        this.rarity = Rarity.EPIC;
        this.attackAnimTime.setBaseValue(30);
    ...
    protected void fireProjectiles(Level level, int x, int y, PlayerMob player, InventoryItem item, int seed, BulletItem bullet, boolean consumeAmmo, PacketReader contentReader) {
        LevelEvent beam1 = new BeamRifleEvent(player, new GameDamage(this.attackDamage.getValue(this.getUpgradeTier(item)),10F,0.05f), 0.2f, player.getX(), player.getY(), x, y, GameRandom.globalRandom.getIntBetween(-5, 5), player, new Color(45, 120, 229),1,347);
        LevelEvent beam2 = new BeamRifleEvent(player, new GameDamage(this.attackDamage.getValue(this.getUpgradeTier(item)),10F,0.05f), 0.2f, player.getX(), player.getY(), x, y, GameRandom.globalRandom.getIntBetween(-5, 5), player, new Color(236, 126, 69),2,347);
        player.getLevel().entityManager.addLevelEventHidden(beam1);
        player.getLevel().entityManager.addLevelEventHidden(beam2);
        if (level.isServer()) {
            level.getServer().network.sendToClientsWithEntityExcept(new PacketLevelEvent(beam1), beam1, player.getServerClient());
            level.getServer().network.sendToClientsWithEntityExcept(new PacketLevelEvent(beam2), beam2, player.getServerClient());
        }
    }
