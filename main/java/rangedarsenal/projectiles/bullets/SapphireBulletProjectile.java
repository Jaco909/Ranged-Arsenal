...
import rangedarsenal.events.CrystalSplosionEvent;
...
public class SapphireBulletProjectile extends BulletProjectile {
    ...
    public SapphireBulletProjectile(float x, float y, float targetX, float targetY, float speed, int distance, GameDamage damage, int knockback, Mob owner) {
        this();
            ...
            if (this.amountHit() >= this.piercing || object != null) {
                this.getLevel().entityManager.addParticle(new ProjectileHitStuckParticle(mob, this, x, y, (float) GameRandom.globalRandom.getIntBetween(10, 20), 5000L) {
                    public void addDrawables(Mob target, float x, float y, float angle, List<LevelSortedDrawable> list, OrderableDrawables tileList, OrderableDrawables topList, Level level, TickManager tickManager, GameCamera camera, PlayerMob perspective) {
                        ...
                        int drawY = camera.getDrawY(y - SapphireBulletProjectile.this.height) - 2;
                        float alpha = 1.0F;
                        ...
                        final TextureDrawOptions options = SapphireBulletProjectile.this.texture.initDraw().light(light).rotate(SapphireBulletProjectile.this.getAngle(), 2, 2).alpha(alpha).pos(drawX, drawY);
